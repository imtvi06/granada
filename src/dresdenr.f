...
C y no mayor que la caja
      ...
      if (qsolid1) then
        DO j=1,2
          ...
          DO i=1,nsolv
            as(j,i) = as(j,i) - ass
            if (as(j,i).gt.(aqpp(j)-dx)) goto 40
            if (as(j,i).lt.(aqpm(j)+dx)) goto 40
          ENDDO
        ENDDO
        CALL RANDOM_NUMBER (Rand)
        ...
        DO i=1,nsolv
          as(3,i) = as(3,i) - ass
          if (as(3,i).lt.aqpp(3).or.as(3,i).gt.(aqpp(3)+dimen)) goto 40
        enddo
      elseif (qsolid2) then
        DO j=1,2
          CALL RANDOM_NUMBER (Rand)
        ...
        ENDDO
        CALL RANDOM_NUMBER (Rand)
        ass = signo()*Rand*abs(apzmax - apzmin)
        DO i=1,nsolv
          as(3,i) = as(3,i) - ass
...
C mas proximos entre si que los parametros de van der Waals
      ...
      CALL TESTVDW (ntot, qroom, aa, nat, nsolv, as, nats, fvdw, *10)
...
*      write (6,'(a)') title1
     ...
     &' CELL:',ii,', central system with',nlim,' distributed molecules.'
      write (6,'(a)') title3
      do i=1,ntot
        write (6,'(a2,3f14.5)') ISYMB(nat(i)), (aa(j,i),j=1,3)
      enddo
      close (6)
...
C Generador aleatorio de signos positivos y negativos
      ...
      SUBROUTINE TESTVDW (np, qroom, aa, natp, ns, as, nats, fvdw, *)
      implicit real*8 (a-h,o,p,r-z)
